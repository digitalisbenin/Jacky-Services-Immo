...
Route::get('/', function (Request $request) {
    ...
    $query = Article::with('site')->where('disponible', '1');
    ...
    if ($request->filled('keyword')) {
        $query->where(function ($q) use ($request) {
            $q->where('name', 'LIKE', '%' . $request->keyword . '%')
              ->orWhere('vedette', 'LIKE', '%' . $request->keyword . '%');
        });
    }

    // Récupération des articles filtrés (avec une limite de 9 articles)
    $article = $query->get();
    return view('accueil', compact('categories','article'));
});
...
Route::get('/parcelle', function (Request $request) {
    $categories = Site::all();

    $query = Article::with('site')->where('disponible', '1')->whereIn('vedette', ['domaine', 'parcelle']);
    ...
    return view('parcelle', compact('categories', 'article'));
});
// Route::get('/accueil', function () {
//     // $commande = Commande::all();
//     $categorie = Site::all();

//     $article = Article::with('site')->where('disponible','1')->take(9)->get();
//     $query = Article::query();
...
Route::get("/habitations", function(Request $request){
    $categories = Site::all();

    $query = Article::with('site')->where('disponible', '1')->where('vedette','villa');
    ...
    return view('habitation', compact('categories', 'article'));
});
Route::get("/details/{id}/article", function($id){
    $article = Article::with('site')->findOrFail($id);
