    ...
    public function send(Request $request)
    {
        ...
        try {
            ...
            Mail::send([], [], function ($message) use ($data) {
                $message->to('contact@jackysercices-immobiliers.com')
                    ->subject('Nouveau message de contact')
                    ->html("
                         <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
                <h2 style='text-align: center; color: #333; margin-bottom: 20px;'>Nouveau message de contact</h2>
                <p style='margin: 0 0 15px;'><strong>Nom :</strong> <span style='color: #555;'>{$data['name']}  {$data['prenom']}</span></p>
                <p style='margin: 0 0 15px;'><strong>Email :</strong> <span style='color: #555;'>{$data['email']}</span></p>
                <p style='margin: 0 0 15px;'><strong>Téléphone :</strong> <span style='color: #555;'>{$data['phone']}</span></p>
